...
int food_menu() {
	...
	cfprt=fopen("fc.txt","a+");
	fprintf(cfprt,"\t\t\t\t\t\t\t CUSTOMER DETAILS\n\nTotal price your foods:%.2f\nTotal price your dessert:%.2f\nYour total Bill is:%.2f\n",Totprice,total,Totprice+total);
...
float Getfood(float totprice,int qrt) {
	...
	cfprt=fopen("fc.txt","a+");
	fprintf(cfprt,"Number of you choose Food Item:%d\nThe Quantity:%d\n",n,qrt);
	return totprice;
}
...
float deasert(float total,int qrt) {
	...
	cfprt=fopen("fc.txt","a+");
	fprintf(cfprt,"Number of you choose Dessert:%d\nThe Quantity:%d\n",n,qrt);
	return total;
}
...
int payment() {
	...
	int tot=0,p_m=0,dis_amnt=0,card,nic,card_no,card_no2,cvc;
	char ch='n';
	char name[30]=" ";
	int confirmpayment,pkg,billopt;
	char ch2='y';
	...
	printf("\n\t\t\t\t    <<<<<<<---------:::\t Select an option to pay your bill\t:::--------->>>>>>>	\n\n\t\t\t\t\t\t\t1.Room Booking\n\t\t\t\t\t\t\t2.Food order\n\n\t\t\t\t\t\t Enter Your choice:  ");
	scanf("%d",&billopt);
	...
	dis_amnt=tot-payable_amount(tot);	//finding the DISCOUNT
...
int payable_amount(int tot) {
	int w_d=0;

	if(tot <= 1500) {
		w_d=tot-85;
	} else if(tot >= 1500 && tot <= 2500) {
		w_d=tot-120;
	} else if(tot >= 2500 && tot <= 3500) {
		w_d=tot-215;
	} else if(tot >= 3500 && tot <= 5000) {
		w_d=tot-358;
	} else if(tot >= 5000 && tot <= 7500) {
		w_d=tot-503;
	} else if(tot >= 7500 && tot <= 9000) {
		w_d=tot-625;
	} else if(tot >= 9000 && tot <= 10000) {
		w_d=tot-701;
	} else if(tot >= 10000 && tot <= 12500) {
		w_d=tot-823;
	} else if(tot >= 12500 && tot <= 15000) {
		w_d=tot-987;
	} else {
		w_d=tot-0;
	}
